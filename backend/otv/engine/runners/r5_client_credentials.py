...
@dataclass
class ClientKey:
    """An EC key pair the client signs client_assertions with (ES256)."""
    ...
    def generate(cls) -> "ClientKey":
        ...
        kid = f"client-key-{uuid.uuid4().hex[:8]}"
        public_jwk = json.loads(jwt.algorithms.ECAlgorithm.to_jwk(key.public_key()))
        public_jwk.update({"kid": kid, "use": "sig", "alg": "ES256"})
        return cls(kid=kid, _private_pem=private_pem, public_jwk=public_jwk)

    def sign_assertion(
        self, *, client_id: str, audience: str, ttl_seconds: int = 60, iat_offset: int = 0
    ) -> Tuple[str, str]:
        """Sign a real client_assertion (RFC 7523 §2.2); returns ``(jwt, jti)``.

        ``iat_offset`` lets a caller mint an assertion as if issued in the past (a
        negative offset) — used to model an artifact captured from an earlier flow
        that is now expired.
        """
...
class ServiceClient:
    ...
    key: ClientKey = field(default_factory=ClientKey.generate)
...
class M2MTokenEndpoint:
    ...
    def _verify_assertion(self, assertion: str) -> Tuple[bool, str]:
        ...
        try:
            public_key = jwt.PyJWK.from_dict(self.registered.key.public_jwk).key
            claims = jwt.decode(
                assertion,
                public_key,
                algorithms=["ES256"],
...
def _client_presented(
    ...
    if method == "private_key_jwt":
        assertion, _ = registered.key.sign_assertion(
            client_id=registered.client_id, audience=TOKEN_URL
        )
