...
discovers and imports every module here so the set of catalogued features is
exactly "the files in this directory".

**To add a capability or attack:** drop a new module in this package that calls
``registry.capability(...)`` / ``registry.attack(...)`` at module scope. Nothing
else — no central list to edit, no picker change. Each item carries an explicit
``order`` that fixes its place in the catalog; the filename is just a name.
"""
